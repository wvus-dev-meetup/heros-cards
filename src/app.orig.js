...
import { Search } from './search.orig.js';
import { Cards } from './cards.orig.js'

const win = window;
const SIZE = 4;
const START_INDEX = SIZE - 1;
...
export default class App extends React.Component {
  ...
  displayLearnMore() {
    return (this.state.visibleIndex < (this.state.profiles.length - 1));
  }

  loadMore() {
    this.setState({visibleIndex: this.state.visibleIndex + SIZE});
    // setTimeout(() => {window.location.href = "#bottom";}, 200);
  }
  ...
  render() {
    return (
      <div className="App">
        ...
        <Cards profiles={this.profiles} searchText={this.state.searchText} selectedType={this.state.selectedType} />
        <div id="bottom"></div>
      </div>
    );
  }
